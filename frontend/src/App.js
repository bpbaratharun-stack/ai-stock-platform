...
import ConfluenceV2 from "./ConfluenceV2";
...
function Navbar() {
  ...
  return <div style={{ display: "flex", gap: 10, flexWrap: "wrap", background: "#0b0f19", padding: "12px 16px", borderRadius: 8, border: "1px solid #1e293b", marginBottom: 24 }}>
    {link("/", "✨ PORTFOLIO")}{link("/profile", "🔎 FACTOR PROFILE")}{link("/screener", "🛰 UNIVERSE SCREENER")}{link("/breakouts", "🚀 WEEKLY BREAKOUTS")}{link("/top", "🏆 TOP PERFORMERS")}{link("/vcp", "🔬 VCP BREAKOUTS")}
  </div>;
}
...
function Shell() {
  ...
  if (loc.pathname === "/v2/common") return <ConfluenceV2 />;
  return (
